        ...
        if($result->num_rows > 0) {
            // output data of each row
             while($row = $result->fetch_assoc()) {
                ...
                $dirname=mysqli_query($conn,$sql2,);
                $row2=mysqli_fetch_assoc($dirname);
                $sql2="SELECT * from production_house where prod_id='$row[prod_id]'";
                $prodname=mysqli_query($conn,$sql2,);
                $row3=mysqli_fetch_assoc($prodname);
                $pos=$row["poster"];
                $list = $list.'
                <div class="row box">
                <div class="col-md-2 box4">
                ...
                <div class="col-md-4 box5">
                <a href="details.php?id='.$row["movie_id"].'pid='.$row3["prod_id"].'" class="btn btn-danger"> More Details</a>
                </div>
            </div>            ';
             }
            }
